        ...
        <div id="card-header-formulario" class="card-header py-3">
          <p class="text-primary m-0 fw-bold d-flex justify-content-between">
            <span class="titulo-formulario">
              <i class="bi bi-gender-female"></i> Antecedentes
              Gineco-Obstétricos
            </span>
          </p>
        </div>
...
export default {
  ...
  setup() {
    ...
    const getValueClass = (key, value) => {
      if (["gestante", "seRealizaCitologias"].includes(key)) {
        return value === "Sí" ? "value-yes" : "value-no";
      }
      if (["gestas", "partos", "abortos"].includes(key)) {
        return parseInt(value) > 0 ? "value-positive" : "value-zero";
      }
      return "";
    };
